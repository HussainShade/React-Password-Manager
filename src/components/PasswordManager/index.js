...
class PasswordManager extends Component {
  ...
  componentDidMount() {
    const storedRecords = localStorage.getItem('passwordRecords')
    if (storedRecords) {
      this.setState({passwordRecords: JSON.parse(storedRecords)})
    }
  }

  componentDidUpdate(prevProps, prevState) {
    const {passwordRecords} = this.state
    if (prevState.passwordRecords !== passwordRecords) {
      localStorage.setItem('passwordRecords', JSON.stringify(passwordRecords))
    }
  }
